# Eon Go Client - AI-Optimized Documentation

This documentation is structured for AI agents to easily understand, parse, and implement the Eon Go client library and CLI.

## Package Information

```yaml
package: github.com/slimcdk/go-eon
version: v1
language: Go
purpose: Interface with Swedish Eon electricity data API
apis:
...
Customer:
  ...
  factory: eon.NewCustomer(refreshToken string)
...
ThirdParty:
  ...
  factory: eon.NewThirdParty(refreshToken string)
...
// INPUT: refreshToken (string) - Obtained from eon.se portal
// OUTPUT: Customer interface
// EXAMPLE:
client := eon.NewCustomer("your-refresh-token-here")
```
...
// EXAMPLE:
client := eon.NewThirdParty("your-refresh-token-here")
```
...
//     ENUM VALUES:
//       - eon.Actual: Raw 15-min meter readings
//       - eon.Quarter: 15-minute aggregation
//       - eon.Hour: Hourly aggregation
//       - eon.Day: Daily aggregation
//       - eon.Month: Monthly aggregation
//       - eon.Year: Yearly aggregation
// DATE CONSTRAINTS:
...
//   - aggregation (Aggregation): Data granularity
// OUTPUTS:
//   - io.ReadCloser: CSV data stream (Swedish format, semicolon-delimited)
//   - error: HTTP/network errors
// CSV FORMAT:
//   - Delimiter: semicolon (;)
//   - Encoding: UTF-8 with BOM
//   - Headers: Swedish language
//   - Columns: MålepunktsID, Fra_dato, Til_dato, Mængde, Måleenhed, Kvalitet, Type
// USAGE PATTERN:
stream, err := client.ExportTimeSeries(ids, from, to, eon.Hour)
if err != nil { /* handle error */ }
defer stream.Close()
...
//     OPTIONS:
//       - eon.AuthorizationIDScope: Use authorization.ID
//       - eon.CustomerCVRScope: Use CVR number
//       - eon.CustomerKeyScope: Use customer key
...
### Command Structure
```
eon --token=<refresh-token> <api-type> <command> [args] [flags]
...
```yaml
Input: CSV stream with semicolon delimiter, UTF-8 BOM, Swedish headers
Process:
...
```go
// 1. Create client
client := eon.NewThirdParty(token)
...
for _, auth := range auths {
    ...
    ts, _ := client.GetTimeSeries(ids, from, to, eon.Hour)
    // Process data...
}
```
...
Export Commands:
  --format=csv: Semicolon-delimited, UTF-8 BOM, Swedish headers
...
import (
    ...
    eon "github.com/slimcdk/go-eon/eon"
)
...
func main() {
    ...
    if token == "" {
        log.Fatal("CLIENT_ID environment variable required")
    }
...
to := time.Now()
ts, _ := client.GetTimeSeries([]string{id}, from, to, eon.Hour)
data := ts[0].Flatten()

// Export to JSON file
stream, _ := client.ExportTimeSeries([]string{id}, from, to, eon.Hour)
